...
import { ClientReport } from "@/lib/types/analytics"
import { initialClientReports } from "@/lib/data/analytics"
...
export default function AnalyticsClientReportsPage() {
  ...
  if (error) {
    return (
      <ErrorState
        title="Failed to load client reports"
        message="We couldn't load client reports. Please check your connection and try again."
        onRetry={() => refetch()}
      />
    )
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-foreground leading-[1.35]">Client Reports</h1>
      ...
      <Card className="border border-border rounded-[14px]">
        <div className="border-b border-border px-5 py-4 flex items-center justify-between">
          <h2 className="text-base font-semibold text-foreground">Client Reports</h2>
          <div className="flex items-center gap-3">
          ...
          {filteredReports.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {filteredReports.map((report) => {
                const type = typeConfig[report.reportType] || typeConfig.custom
            ...
            <EmptyState
              ...
              description="Generate your first client report to get started."
              action={{
