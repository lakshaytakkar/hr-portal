...
import { EmailAutomation } from "@/lib/types/marketing"
import { initialEmailAutomations } from "@/lib/data/marketing"
import { EmptyState } from "@/components/ui/empty-state"
import { ErrorState } from "@/components/ui/error-state"
import { Skeleton } from "@/components/ui/skeleton"
import { RowActionsMenu } from "@/components/actions/RowActionsMenu"
import { CreateAutomationDialog } from "@/components/marketing/CreateAutomationDialog"
import { getAvatarForUser } from "@/lib/utils/avatars"

async function fetchEmailAutomations() {
  await new Promise((resolve) => setTimeout(resolve, 500))
  return initialEmailAutomations
}
...
export default function MarketingEmailAutomationsPage() {
  ...
  if (error) {
    return (
      <ErrorState
        title="Failed to load email automations"
        message="We couldn't load email automations. Please check your connection and try again."
        onRetry={() => refetch()}
      />
    )
  }
    ...
    <div className="space-y-5">
      ...
      <Card className="border border-border rounded-[14px]">
        <div className="flex h-16 items-center justify-between border-b border-border px-5 py-2 bg-white">
          <h2 className="text-base font-semibold text-foreground">Email Automations</h2>
              ...
              {filteredAutomations.length > 0 ? (
                    ...
                    <TableRow key={automation.id} className="border-b border-border">
                      ...
                      <TableCell className="px-3">
                        <span className="text-sm font-medium text-foreground">
                          {automation.opened || 0} ({automation.recipients ? Math.round(((automation.opened || 0) / automation.recipients) * 100) : 0}%)
                        </span>
                      </TableCell>
                      <TableCell className="px-3">
                ...
                <TableRow>
                  <TableCell colSpan={8} className="h-24">
                    <EmptyState
                      icon={Mail}
                      title="No email automations yet"
                      description="Create automated email workflows to engage leads automatically."
                      action={{
      ...
      <CreateAutomationDialog open={isCreateAutomationOpen} onOpenChange={setIsCreateAutomationOpen} />
    </div>
  )
}
