...
export function EditCallDialog({ open, onOpenChange, call }: EditCallDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    try {
      console.log("Update call:", formData)
      toast.success("Call updated successfully", {
        description: `Call with **${formData.contactName || "contact"}** has been updated`,
        duration: 3000,
      })
      onOpenChange(false)
    } catch (error) {
